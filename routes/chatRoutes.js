...
Chat.belongsToMany(User, { through: ChatParticipante, foreignKey: 'chat_id', otherKey: 'usuario_id' });
User.belongsToMany(Chat, { through: ChatParticipante, foreignKey: 'usuario_id', otherKey: 'chat_id' });
...
Chat.hasMany(ChatParticipante, { foreignKey: 'chat_id', as: 'participantes' });
User.hasMany(ChatParticipante, { foreignKey: 'usuario_id', as: 'chats_participante' });
...
app.use('/api', chatRoutes); // Rotas do chat (GET /api/chats, POST /api/chats/usuario/:id, etc.)
...
app.get('/api/chats-test', (req, res) => {
    res.json({ mensagem: 'Rota de teste do chat funcionando!' });
});
